...
- (void)drawLinearGradient:(CGContextRef _Nullable )context
                ...
                startColor:(CGColorRef _Nullable )startColor
                  endColor:(CGColorRef _Nullable )endColor;

/**
 设置控件阴影样式
 */
- (void)setViewShadowColor:(UIColor *_Nullable)shadowColor
 ...
 *  @param raduis   需要设置的圆角大小 例如 CGSizeMake(20.0f, 20.0f)
 */
- (void)addRoundedCorners:(UIRectCorner)corners
               withRaduis:(CGSize)raduis;
/**
 *  设置部分圆角(相对布局)
 *
 *  @param corners 需要设置为圆角的角 UIRectCornerTopLeft | UIRectCornerTopRight | UIRectCornerBottomLeft | UIRectCornerBottomRight | UIRectCornerAllCorners
